    ...
    if (spotImg) {
        spotImg.addEventListener('click', () => {
            if (currentSpotlightProduct) {
                openModal(currentSpotlightProduct, currentSpotlightVariantIndex);
            }
        });
    }
}
...
function renderCart() {
    ...
    cart.slice().reverse().forEach(item => {
        ...
        const img = item.variant && item.variant.featured_image ? item.variant.featured_image.src : getProductImage(item.product);
        ...
        el.innerHTML = `
            ...
            <div class="cart-item-details">
                <div>
                    ...
                    <p class="cart-item-variant">${color} / ${item.size}</p>
...
function openCheckout() {
    ...
    // FX Removed
    // if (window.cardScanner) window.cardScanner.start();
    ...
    cart.forEach(item => {
        ...
        const title = cleanProductTitle(item.product.title);
        const color = item.variant ? item.variant.option1 : 'Default';
        const priceVal = item.variant ? parseFloat(item.variant.price) : 0;
        ...
        row.innerHTML = `
            ...
            <div class="chk-details">
                <h4 class="chk-title">${title}</h4>
                <p class="chk-variant">${color} / ${item.size}</p>
            </div>
            <div class="chk-price">$${priceVal.toFixed(2)}</div>
        `;
...
function processMockPayment(e) {
    ...
    setTimeout(() => {
        ...
        // Stop FX
        // if (window.cardScanner) window.cardScanner.stop();
    ...
    if (colors.length > 0) {
        colors.forEach((color, idx) => {
            ...
            thumb.onclick = () => {
                ...
                const matchedVariant = product.variants.find(v => v.option1 === color);
                if (matchedVariant) {
                    price.innerText = formatPrice(matchedVariant);
                }
            };
...
function getVariantImage(product, colorName) {
    // 1. Try to find a variant with this option1 value and a valid featured_image
    const v = product.variants.find(v => v.option1 === colorName && v.featured_image);
    if (v && v.featured_image.src) return v.featured_image.src;

    // 2. Fallback: Check product.images alt text? Or just return main image
...
function addToBag() {
    ...
    addToCart(product, variant, finalSize);
...
function renderWheels() {
    ...
    for (let i = 0; i < NUM_WHEELS; i++) {
        const chunk = window.railProducts.slice(i * CHUNK_SIZE, (i + 1) * CHUNK_SIZE);
...
function renderGrid() {
    ...
    products.forEach(p => {
        ...
        card.onclick = () => {
            // Smart Detection: Find variant index that matches the image we are showing
            // ROBUST FIX: Match by Image ID (Source of Truth), NOT URL string.
            // The grid shows p.images[0]. If this image corresponds to a variant, we MUST open that variant.

            let targetIdx = 0;
            const displayedImageId = (p.images && p.images[0]) ? p.images[0].id : null;

            if (displayedImageId && p.variants) {
                // Find variant with matching image_id
                const foundIdx = p.variants.findIndex(v => v.image_id === displayedImageId);

                if (foundIdx !== -1) {
                    targetIdx = foundIdx;
                    console.log(`[VariantLock] Matched ImageID ${displayedImageId} to Variant Index ${targetIdx}`);
                } else {
                    console.log(`[VariantLock] ImageID ${displayedImageId} not linked to specific variant. Defaulting to 0.`);
                }
            }
            // ROBUST FIX: Pass 'src' (the image currently shown) to ensure modal matches VISUALLY 100%
            openModal(p, targetIdx, src);
        };
