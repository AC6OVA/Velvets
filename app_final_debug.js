...
function enforceMainVariantFirst() {
    ...
    products.forEach(product => {
        ...
        // 1. Identify Main Image (The Source of Truth)
        const mainImageSrc = typeof product.images[0] === 'string' ? product.images[0] : product.images[0].src;

        // 2. Check if Variant 0 matches this image
        const v0 = product.variants[0];
        const v0Image = v0.image || (v0.featured_image ? v0.featured_image.src : null);

        // Normalize for comparison (ignore query params if possible, or simple includes)
        const match = v0Image === mainImageSrc;

        if (!match) {
            // 3. Find the Correct Variant
            let correctIndex = -1;

            // A. Specific Fix for nc_004 (Blue Main Image, but Camel V0)
            if (product.id === 'nc_004') {
                // Main Image is Blue. Find "Blue" variant.
                correctIndex = product.variants.findIndex(v => v.option1 && v.option1.toLowerCase() === 'blue');
            } else if (product.id === 'nc_001') {
                // Fix for nc_001: Main Image is Blue Denim. Variant 0 is Camel.
                // Find "Sky Blue" or "Blue"
                correctIndex = product.variants.findIndex(v => v.option1 && v.option1.toUpperCase().includes('BLUE'));
            } else {
                // B. Generic Image Match
            ...
            if (correctIndex > 0) {
                ...
                const temp = product.variants[0];
                product.variants[0] = product.variants[correctIndex];
                product.variants[correctIndex] = temp;

                // CRITICAL: Ensure the swap persists in all references
                // (Javascript arrays are reference info, so this is good)
            }
        }
    });
}
...
function openCheckout() {
    ...
    // Render Items
    // Just calculate total for the button
    let subtotal = 0;
    let totalWeight = 0; // grams

    cart.forEach(item => {
        const priceVal = item.variant ? parseFloat(item.variant.price) : 0;
        const weightVal = item.variant ? (item.variant.weight || 0) : 0;
    ...
    // Shipping Logic
    // < 1000g (1kg) = Economy ($6.99)
    // >= 1000g = Standard ($12.99)
    ...
    if (subtotal < 100) {
        if (totalWeight < 1000) {
            shippingCost = 6.99;
            shippingLabel = "Economy Shipping";
        } else {
            shippingCost = 12.99;
    ...
    // Update Pay Button with Breakdown (Optional: you might want to show this elsewhere)
    // For now, simpler is better for the button text, maybe show label?
    const payBtn = document.getElementById('completeOrderBtn');
    if (payBtn) {
        // Create a summary above the button if it doesn't exist, or just update button
        // Let's just update button text to be clear
        payBtn.innerHTML = `PAY $${total.toFixed(2)} <span style="font-size: 0.8em; opacity: 0.8">(${shippingLabel}: $${shippingCost.toFixed(2)})</span>`;
    }
}
